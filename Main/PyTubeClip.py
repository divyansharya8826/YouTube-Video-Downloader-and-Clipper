from tkinter import (Button, Canvas, PhotoImage, StringVar,
                     Tk, ttk, filedialog, OptionMenu, Image)
...
img = Image("photo", file="Divyansh/Python/Youtube Video Downloader and Clipper/Images/Babasse-Old-School-Bobines-video.256.png")
root.tk.call('wm','iconphoto', root._w, img)
root.title("YouTube Video Cliper")
bg = PhotoImage(
    file="Divyansh/Python/Youtube Video Downloader and Clipper/Images/i.ppm")
bg2 = PhotoImage(
    file="Divyansh/Python/Youtube Video Downloader and Clipper/Images/youtubevanced.png")
canvas1 = Canvas(root, width=1024, height=768)
canvas1.pack(fill="both", expand=True)
canvas1.create_image(0, 0, image=bg, anchor="nw")
canvas1.create_image(0, -65, image=bg2, anchor="nw")

l1 = canvas1.create_text((400, 50), text="Youtube Video Cliper", font=(
    'Helvetica', 30, 'bold', 'underline'), fill="red")
# enter link
link = StringVar()

l2 = canvas1.create_text((400, 115), text='Paste Link Here:', font=(
...
def get_save_path():
    ...
    folder = filedialog.asksaveasfilename(
        initialdir="/", initialfile=yt.title, title="save the file", defaultextension=".mp4")
...
# for clipping video
l3 = canvas1.create_text((100, 215), text='Start',
                         font=('arial', 20, 'bold'), fill='red')
Start = StringVar()
start_time = ttk.Entry(root, width=10, textvariable=Start).place(x=150, y=200)
l4 = canvas1.create_text((550, 215), text='End',
                         font=('arial', 20, 'bold'), fill='red')
End = StringVar()
end_time = ttk.Entry(root, width=10, textvariable=End).place(x=600, y=200)

l5 = canvas1.create_text((325, 215), text='Resolution',
                         font=('arial', 20, 'bold'), fill='red')
Resolution = StringVar()
Resolution.set("----")
Res = OptionMenu(root, Resolution, "240p", "360p", "480p",
                 "720p", "1080p").place(x=400, y=205)
...
def clip_video():
    ...
    if (progress['value'] == 100):
        l5 = canvas1.create_text((400, 400), text='DOWNLOADED', font=(
    ...
    clip.write_videofile(filename=folder, codec='libx264',
                         fps=30, audio=True, audio_codec="aac")


Download = Button(root, text='DOWNLOAD', font=('arial', 15, 'bold'), background="blue",
                  padx=10, command=lambda: [get_save_path(), clip_video()], pady=3).place(x=340, y=240)
...
bton_for_whole_video = Button(root, text="DOWNLOAD\nFULL VIDEO", font=(
    'arial', 15, 'bold'), bg="red", padx=10, command=lambda: [get_save_path(), Downloader()]).place(x=340, y=280)
